...
if (global.Hit){
	...
	if !(instance_exists(obj_enemyEyesPink)){
		visible= true;
	}
}
	...
	if(mode == "caza"){
		...
		if(direction != 180){
			if (place_meeting(x+2, y, walls) == false){
				var dist = point_distance(x+20,y,obj_player.x+dx,obj_player.y+dy);
		...
		if(direction != 270){
			if (place_meeting(x, y-2, walls) == false){
				var dist = point_distance(x,y-20,obj_player.x+dx,obj_player.y+dy);
				if(dist<distance_to_pacman){
					new_direction=90;
					distance_to_pacman=dist;
				}
